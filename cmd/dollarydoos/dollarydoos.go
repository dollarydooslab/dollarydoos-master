...
import (
	...
	"github.com/dollarydooslab/dollarydoos/src/util/browser"
	"github.com/dollarydooslab/dollarydoos/src/util/cert"
	"github.com/dollarydooslab/dollarydoos/src/util/encrypt"
...
type Config struct {
	...
	// Data directory holds app data -- defaults to ~/.dollarydoos
...
func (c *Config) register() {
	...
	flag.StringVar(&c.DataDirectory, "data-dir", c.DataDirectory, "directory to store app data (defaults to ~/.dollarydoos)")
	flag.StringVar(&c.DBPath, "db-path", c.DBPath, "path of database file (defaults to ~/.dollarydoos/data.db)")
	...
	flag.StringVar(&c.WalletDirectory, "wallet-dir", c.WalletDirectory, "location of the wallet files. Defaults to ~/.dollarydoos/wallet/")
...
var devConfig = Config{
	...
	PeerListURL:            "https://www.dollarydoos.io/blockchain/peers.txt",
	...
	LaunchBrowser: false,
	// Data directory holds app data -- defaults to ~/.dollarydoos
	DataDirectory: filepath.Join(home, ".dollarydoos"),
	...
	ProfileCPU: false,
	// Where the file is written to
	ProfileCPUFile: "dollarydoos.prof",
	// HTTP profiling interface (see http://golang.org/pkg/net/http/pprof/)
	HTTPProf: false,
	// Will force it to connect to this ip:port, instead of waiting for it
	// to show up as a peer
...
func createGUI(c *Config, d *daemon.Daemon, host string, quit chan struct{}) (*gui.Server, error) {
	...
	if c.WebInterfaceHTTPS {
		...
		if err := cert.CreateCertIfNotExists(host, c.WebInterfaceCert, c.WebInterfaceKey, "dollarydoosd"); err != nil {
...
// Run starts the dollarydoos node
func Run(c *Config) {
	defer func() {
	...
	if err != nil {
		logger.Errorf("Database failed to open: %v. Is another dollarydoos instance running?", err)
		return
	}
