...
CLI will connect to dollarydoos node rpc address:`127.0.0.1:8650` by default,
...
The default CLI wallet dir is located in `$HOME/.dollarydoos/wallets/`, change it by setting the
...
The default CLI wallet file name is `dollarydoos_cli.wlt`, change it by setting the `WALLET_NAME` env.
...
After the installation, you can run `dollarydoos-cli` to see the usage:

```
$ dollarydoos-cli

NAME:
   dollarydoos-cli - the dollarydoos command line interface

USAGE:
   dollarydoos-cli [global options] command [command options] [arguments...]
...
COMMANDS:
     ...
     addressGen            Generate dollarydoos or bitcoin addresses
     ...
     send                  Send dollarydoos from a wallet or an address to a recipient address
     status                Check the status of current dollarydoos node
     transaction           Show detail info of specific transaction
     verifyAddress         Verify a dollarydoos address
     version
     walletBalance         Check the balance of a wallet
     walletDir             Displays wallet folder address
     walletHistory         Display the transaction history of specific wallet. Requires dollarydoos node rpc.
...
ENVIRONMENT VARIABLES:
    ...
    COIN: Name of the coin. Default "dollarydoos"
...
### Add Private Key
Add a private key to a dollarydoos wallet.

```bash
$ dollarydoos-cli addPrivateKey [command options] [private key]
```

```
OPTIONS:
    -f value [wallet file or path] private key will be added to this wallet
    if not specified then default wallet ($HOME/.dollarydoos/wallets//wallets/dollarydoos_cli.wlt)
    will be used
```

#### Example
```bash
$ dollarydoos-cli addPrivateKey -f $WALLET_PATH $PRIVATE_KEY
```
...
```bash
$ dollarydoos-cli addressBalance [addresses]
```

#### Example
```bash
$ dollarydoos-cli addressBalance 2iVtHS5ye99Km5PonsB42No3pQRGEURmxyc 2GgFvqoyk9RjwVzj8tqfcXVXB4orBwoc9qv
```
<details>
...
Generate new dollarydoos or bitcoin addresses.

```bash
$ dollarydoos-cli addressGen [command options] [arguments...]
```
...
OPTIONS:
        ...
        --bitcoin, -b            Output the addresses as bitcoin addresses instead of dollarydoos addresses
...
#### Examples
##### Generate `n` number of dollarydoos addresses
```bash
$ dollarydoos-cli addressGen --count 2
```
...
```bash
$ dollarydoos-cli addressGen --count 2 --bitcoin
```
...
```bash
$ dollarydoos-cli addressGen --count 2 --hide-secret
```
...
```bash
$ dollarydoos-cli addressGen --count 2 --only-addr
```
...
```bash
$ dollarydoos-cli addressGen --count 2 --seed "my super secret seed"
```
...
```bash
dollarydoos-cli addressGen --count 2 --hex
```
...
```bash
$ dollarydoos-cli addressOutputs [address list]
```

#### Example
```bash
dollarydoos-cli addressOutputs tWPDM36ex9zLjJw1aPMfYTVPbYgkL2Xp9V 29fDBQuJs2MDLymJsjyWH6rDjsyv995SrGU
```
...
```bash
$ dollarydoos-cli blocks [starting block or single block seq] [ending block seq]
```

#### Example
```bash
$ dollarydoos-cli blocks 41 42
```
...
Checks if the given database file contains valid dollarydoos blockchain data
If no argument is given, the default `data.db` in `$HOME/.$COIN/` will be checked.

```bash
$ dollarydoos-cli checkdb [db path]
```

#### Example
```bash
$ dollarydoos-cli checkdb $DB_PATH
```
...
```bash
$ dollarydoos-cli createRawTransaction [command options] [to address] [amount]
```
...
```bash
$ dollarydoos-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending to a specific change address

```bash
$ dollarydoos-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ dollarydoos-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
```bash
$ dollarydoos-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ dollarydoos-cli decodeRawTransaction [raw transaction]
```

Decode a raw dollarydoos transaction.

#### Example

```bash
dollarydoos-cli decodeRawTransaction 
```
...
Broadcast a raw dollarydoos transaction.
Output is the transaction id.

```bash
$ dollarydoos-cli broadcastTransaction [raw transaction]
```

```bash
$ dollarydoos-cli broadcastTransaction 
```
<details>
 <summary>View Output</summary>
...
### Generate a wallet
Generate a new dollarydoos wallet.

```bash
$ dollarydoos-cli generateWallet [command options]
```
...
OPTIONS:
        ...
        -f value  [walletName] Name of wallet. The final format will be "yourName.wlt".
                             If no wallet name is specified a generic name will be selected. (default: "dollarydoos_cli.wlt")
...
```bash
$ dollarydoos-cli generateWallet
```
...
```bash
$ dollarydoos-cli generateWallet -r
```
...
```bash
$ dollarydoos-cli generateWallet -rd
```
...
```bash
$ dollarydoos-cli generateWallet -s "this is the super secret seed everyone needs but does not have"
```
...
```bash
$ dollarydoos-cli generateWallet -n 2
```
...
```bash
$ dollarydoos-cli generateWallet -f "secret_wallet.wlt"
```
...
```bash
$ dollarydoos-cli generateWallet -l "cli wallet"
```
...
Generate new addresses for a dollarydoos wallet.

```bash
$ dollarydoos-cli generateAddresses [command options]
```

```
OPTIONS:
        -n value    [numberOfAddresses]  Number of addresses to generate (default: 1)
        -f value    [wallet file or path] Generate addresses in the wallet (default: $HOME/.dollarydoos/wallets//wallets/dollarydoos_cli.wlt)
        --json, -j  Returns the results in JSON format
```

##### Generate an address for the default wallet
```bash
$ dollarydoos-cli generateAddresses
```
...
```bash
$ dollarydoos-cli generateAddresses $WALLET_PATH
```
...
```bash
$ dollarydoos-cli generateAddresses -n 2
```
...
##### Generate a JSON output
```bash
$ dollarydoos-cli generateAddresses --json
```
...
### Last blocks
Show the last `n` dollarydoos blocks.
By default the last block is shown.

```bash
$  dollarydoos-cli lastBlocks [numberOfBlocks]
```

#### Examples
##### Get the last block
```bash
$ dollarydoos-cli lastBlocks
```
...
List addresses in a dollarydoos wallet.

```bash
$ dollarydoos-cli listAddresses [walletName]
```

If no `walletName` is given then default wallet ($HOME/.dollarydoos/wallets/dollarydoos_cli.wlt) is used.

> NOTE: The wallet name `dollarydoos_cli.wlt` or full path `$HOME/.dollarydoos/wallets/dollarydoos_cli.wlt` can be used.
        When only the wallet name is given then the default wallet dir, $HOME/.$COIN/wallets is used.

#### Examples
##### List addresses of default wallet
```bash
$ dollarydoos-cli listAddresses
```
...
```bash
$ dollarydoos-cli listAddresses $WALLET_NAME or $WALLET_PATH
```
...
### List wallets
List wallets in the dollarydoos wallet directory.

```bash
$ dollarydoos-cli listWallets
```
...
### Send
Make a dollarydoos transaction.

```bash
$ dollarydoos-cli send [command options] [to address] [amount]
```

```
OPTIONS:
        -f value    [wallet file or path] From wallet. If no path is specified your default wallet
                    (`$HOME/.dollarydoos/wallets/dollarydoos_cli.wlt`) path will be used.
        -a value    [address] From address
...
```bash
$ dollarydoos-cli send $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending from a specific wallet
```bash
$ dollarydoos-cli send -f $WALLET_PATH $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending from a specific address in a wallet
```bash
$ dollarydoos-cli send -f $WALLET_PATH -a $FROM_ADDRRESS $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ dollarydoos-cli send -f $WALLET_PATH -a $FROM_ADDRESS -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending to multiple addresses
```bash
$ dollarydoos-cli send -f $WALLET_PATH -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
##### Generate a JSON output
```bash
$ dollarydoos-cli send -f $WALLET_PATH -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ dollarydoos-cli transaction [transaction id]
```

#### Example
```bash
$ dollarydoos-cli transaction 824d421a25f81aa7565d042a54b3e1e8fdc58bed4eefe8f8a90748da6d77d135
```
...
### Verify address
Verify whether a given address is a valid dollarydoos addres or not.

```bash
$  dollarydoos-cli verifyAddress [dollarydoos address]
```
...
```bash
$ dollarydoos-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda
```
...
###### Invalid checksum
```bash
$ dollarydoos-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsdx
```
...
```bash
$ dollarydoos-cli verifyAddress 21YPg
```
...
Check the wallet a dollarydoos wallet.

```bash
$ dollarydoos-cli walletBalance [wallet]
```

> NOTE: Both the full wallet path or only the wallet name can be used.
        If no wallet is specified then the default wallet: `$HOME/.$COIN/wallets/dollarydoos_cli.wlt` is used.

#### Example
##### Balance of default wallet
```bash
$ dollarydoos-cli walletBalance
```
...
```bash
$ dollarydoos-cli walletBalance 2018_04_01_198c.wlt
```
*OR*

```bash
$ dollarydoos-cli walletBalance ~/.dollarydoos/wallets/2018_04_01_198c.wlt
```
...
Get the current dollarydoos wallet directory.

```bash
$ dollarydoos-cli walletDir [command options]
```
...
##### JSON output
```bash
$ dollarydoos-cli walletDir --json
```
...
```bash
$ dollarydoos-cli walletHistory [command options]
```
...
##### Default wallet
```bash
$ dollarydoos-cli walletHistory
```

##### Specific wallet
```bash
$ dollarydoos-cli walletHistory -f $WALLET_NAME
```
*OR*
```bash
$ dollarydoos-cli walletHistory -f $WALLET_PATH
```
...
```bash
$ dollarydoos-cli walletOutputs [wallet file]
```

#### Examples
##### Default wallet
```bash
$ dollarydoos-cli walletOutputs
```

##### Specific wallet
```bash
$ dollarydoos-cli walletHistory $WALLET_NAME
```
*OR*
```bash
$ dollarydoos-cli walletHistory $WALLET_PATH
```
...
### CLI version
Get version of current dollarydoos cli.

```bash
$ dollarydoos-cli version [command options]
```
...
```bash
$ dollarydoos-cli send $RECIPIENT_ADDRESS $AMOUNT -c $CHANGE_ADDRESS
```
...
```bash
$ dollarydoos-cli send -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```
