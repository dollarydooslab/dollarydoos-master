...
- go1.10 support
- Add Dockerfile
- Add libdollarydoos C API wrapper
- New wallet UI
...
- For electron clients, download a list of peers from https://downloads.dollarydoos.net/blockchain/peers.txt by default
...
- Add `/logs` api to filter dollarydoos logs, so that we can add a debug panel to the GUI wallet to show logs
...
- CLI's `walletHistory` command prints amounts as fixed-point decimal strings. Previously, it printed amounts as integers representing whole dollarydoos amounts, and did not support droplets / fractional dollarydooss.
...
- #383 Error during installation from dollarydoos source code
