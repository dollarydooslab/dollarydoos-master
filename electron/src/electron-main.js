...
app.setAsDefaultProtocolClient('dollarydoos');
...
var dollarydoos = null;

function startdollarydoos() {
  console.log('Starting dollarydoos from electron');

  if (dollarydoos) {
    console.log('dollarydoos already running');
    app.emit('dollarydoos-ready');
    return
  }
  ...
  // Resolve dollarydoos binary location
  var appPath = app.getPath('exe');
  var exe = (() => {
        switch (process.platform) {
  case 'darwin':
    return path.join(appPath, '../../Resources/app/dollarydoos');
  case 'win32':
    // Use only the relative path on windows due to short path length
    // limits
    return './resources/app/dollarydoos.exe';
  case 'linux':
    return path.join(path.dirname(appPath), './resources/app/dollarydoos');
  default:
    return './resources/app/dollarydoos';
  }
})()
  ...
  dollarydoos = childProcess.spawn(exe, args);

  dollarydoos.on('error', (e) => {
    dialog.showErrorBox('Failed to start dollarydoos', e.toString());
    app.quit();
  });

  dollarydoos.stdout.on('data', (data) => {
    ...
    app.emit('dollarydoos-ready', { url: currentURL });
  });

  dollarydoos.stderr.on('data', (data) => {
    console.log(data.toString());
  });

  dollarydoos.on('close', (code) => {
    // log.info('dollarydoos closed');
    console.log('dollarydoos closed');
    reset();
  });

  dollarydoos.on('exit', (code) => {
    // log.info('dollarydoos exited');
    console.log('dollarydoos exited');
    reset();
  });
}
...
function createWindow(url) {
  ...
  ses.clearCache(function () {
    console.log('Cleared the caching of the dollarydoos wallet.');
  });
  ...
  var template = [{
    label: "dollarydoos",
    submenu: [
      { label: "About dollarydoos", selector: "orderFrontStandardAboutPanel:" },
      { type: "separator" },
...
app.on('ready', startdollarydoos);

app.on('dollarydoos-ready', (e) => {
  createWindow(e.url);
});
