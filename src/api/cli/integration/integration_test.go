...
func TestVerifyAddress(t *testing.T) {
	...
	tt := []struct {
		...
		{
			"valid dollarydoos address",
			"2Kg3eRXUhY6hrDZvNGB99DKahtrPDQ1W9vN",
			nil,
			"",
		},
		{
			"invalid dollarydoos address",
	...
	tt := []struct {
	...
	}{
		{
			...
			func(t *testing.T, v []byte) {
				...
				// Confirms the wallet type is dollarydoos
				require.Equal(t, "dollarydoos", w.Meta["coin"])
...
func TestLiveSend(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			errMsg:  []byte("ERROR: Transaction has zero coinhour fee. See 'dollarydoos-cli send --help'"),
			checkTx: func(t *testing.T, txid string) {},
		},
	}
	...
	errMsg := []byte("ERROR: invalid amount, too many decimal places. See 'dollarydoos-cli send --help'")
...
func TestLiveCreateAndBroadcastRawTransaction(t *testing.T) {
	...
	// Send with too small decimal value
	errMsg := []byte("ERROR: invalid amount, too many decimal places. See 'dollarydoos-cli createRawTransaction --help'")
	for i := uint64(1); i < uint64(20); i++ {
		v, err := droplet.ToString(i)
		require.NoError(t, err)
		name := fmt.Sprintf("send %v", v)
...
func TestVersion(t *testing.T) {
	...
	var ver = struct {
		dollarydoos  string `json:"dollarydoos"`
		Cli    string `json:"cli"`
		RPC    string `json:"rpc"`
	...
	require.NoError(t, err)
	require.True(t, ver.dollarydoos != "")
...
func TestStableGenerateWallet(t *testing.T) {
	...
	tt := []struct {
	...
	}{
		{
			...
			checkWallet: func(t *testing.T, w *wallet.Wallet) {
				// Confirms the default wallet name is dollarydoos_cli.wlt
				require.Equal(t, "dollarydoos_cli.wlt", w.Filename())
		...
		},
		{
			...
			errMsg: []byte("ERROR: integration-test.wlt already exist. See 'dollarydoos-cli generateWallet --help'"),
		},
	}
